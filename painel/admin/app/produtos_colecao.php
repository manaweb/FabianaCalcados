<? 
	...
	if ($_GET['faz']=="dados") {
		...
		# Testes
		$Erros='';
		if (strlen($nome) < 2) $Erros .= "Por favor, insira a cole&ccedil;&atilde;o";
	...
	if ($_GET['faz']=="excluir") {
		...
		if ($id>0) {
			...
			$resultCategorias = db_consulta("select * from tbprodutos_colecao where id_colecao = ".$id);
			while($dados = mysql_fetch_array($resultSubcategorias)){
				@db_consulta("DELETE FROM tbprodutos where id_categoria = ".$dados['id_categoria']);
				@db_consulta("DELETE FROM tbprodutos_categorias where id_categoria = ".$dados['id_categoria']);	
			} 
