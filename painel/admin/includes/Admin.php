...
function adminCampos2($campos,$config,$dados, $idOrcamento) {
	...
	$saida .= '<form name="frmDados" method="post" enctype="multipart/form-data">';
	$saida .= '<input type="hidden" name="'.$config['id'].'" value="'.$dados[$config['id']].'">';
	$saida .= '<fieldset class="adicionar"><legend>Informa&ccedil;&otilde;es</legend>';
	$resultProdutos = db_consulta("SELECT p.nome, it.quantidade, it.variacao
								FROM tbprodutos p, tbitensorcamento it
								where it.id_orcamento = $idOrcamento
								and it.id_produto = p.id");
	$saida .= '
	...
	while($dadosProduto = mysql_fetch_array($resultProdutos)){
		$saida .=
						'<tr>
							<td class="text-left">'.$dadosProduto['nome'].'</td>
	...
	$saida.=		
				...
				<div class="botoes-orcamento">
					<button type="button" class="bt-orcamento responder">Responder Or&ccedil;amento</button> &nbsp;&nbsp;
					<button type="button" class="bt-orcamento dados-cliente">Dados do Cliente</button>
				</div>
				<table class="tb-dados-cliente" style="display: none;">';
...
*/
	...
	$sql = "select o.id_cliente, c.email from tborcamento o, tbclientes c where o.id = ".$_GET['ID']." and c.id = o.id_cliente";
	$dados = mysql_fetch_assoc(mysql_query($sql));
	$saida .= '</table></fieldset></form>

				<div class="responder-orcamento" style="display: none;">
					<h2>Responder Or&ccedil;amento</h2>
					<form name="frmResponderOrcamento" action="../app/responderOrcamento.php" class="adicionar" method="post">
						<input type="hidden" value="'.$dados['id_cliente'].'" name="id_cliente" />
						<input type="hidden" value="'.$_GET['ID'].'" name="id_orcamento" />
						<input type="hidden" value="'.$dados['email'].'" name="email" />
						<table>
							<tr>
								<td>Seu Nome:</td>
								<td><input type="text" name="nome" style="width: 200px;"/></td>
							</tr>
							<tr>
								<td>Status do Or&ccedil;amento</td>
								<td>
									<select name="status-orcamento">
										<option selected value="">Status do Or&ccedil;amento</option>
							...
							<tr>
								...
								<td><textarea id="wysiwyg" name="descricao"></textarea></td>
							</tr>
						</table>
	';
	$saida .= '<div class="botoes"><input type="submit" style="height:35px!important; margin: 0 0 20px 389px;" height="35px" id="btn" value="enviar"   /></div>';
	$saida .= '</form></div>';
